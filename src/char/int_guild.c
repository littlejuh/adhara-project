...
static int guild_save_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	for( g = (struct guild*)iter->first(iter,&key); iter->exists(iter); g = (struct guild*)iter->next(iter,&key) )
	{
		...
		{// Nothing to save, guild is ready for removal.
			if (save_log)
				ShowInfo("Guilda descarregada (%d - %s)\n", g->guild_id, g->name);
...
#ifdef NOISY
	ShowInfo("Salvamento de guilda requisitado ("CL_BOLD"%d"CL_RESET" - flag 0x%x).",g->guild_id, flag);
#endif
...
struct guild * inter_guild_fromsql(int guild_id)
{
...
#ifdef NOISY
	ShowInfo("Carregamento de guilda requisitado (%d)...\n", guild_id);
#endif
	...
	{	// Fix reduction of MAX_GUILD [PoW]
		ShowWarning("Guild %d:%s especifica capacidade (%d) maior do que MAX_GUILD (%d)\n", guild_id, g->name, g->max_member, MAX_GUILD);
...
int inter_guildcastle_tosql(struct guild_castle *gc)
{
	...
	if (gc==NULL) return 0;
	#ifdef GUILD_DEBUG
ShowDebug("guild_castle salvo (%d)\n", gc->castle_id);
	#endif
...
int inter_guildcastle_fromsql(int castle_id,struct guild_castle *gc)
{
	...
	if( save_log )
		ShowInfo("Castelo carregado %d (guilda %d)\n", castle_id, gc->guild_id);
...
int inter_guild_CharOnline(int char_id, int guild_id)
{
   ...
   int i;
	...
	if(!g) {
		ShowError("Guilda %d do personagem %d nao encontrada!\n", guild_id, char_id);
		return 0;
	}
...
int guild_calcinfo(struct guild *g)
{
	...
	if(g->max_member > MAX_GUILD)
	{
		ShowError("Guilda %d:%s tem capacidade para muitos membros (%d), o maximo suportado e %d\n", g->guild_id, g->name, g->max_member, MAX_GUILD);
		g->max_member = MAX_GUILD;
	}
...
int mapif_guild_created(int fd,int account_id,struct guild *g)
{
	...
	if(g != NULL)
	{
		...
		ShowInfo("int_guild: Guilda criada (%d - %s)\n",g->guild_id,g->name);
	} else
		WFIFOL(fd,6)=0;
...
int mapif_guild_withdraw(int guild_id,int account_id,int char_id,int flag, const char *name, const char *mes)
{
	...
	ShowInfo("int_guild: guilda abandonada (%d - %d: %s - %s)\n",guild_id,account_id,name,mes);
	return 0;
}
...
int mapif_guild_broken(int guild_id,int flag)
{
	...
	WBUFB(buf,6)=flag;
	mapif_sendall(buf,7);
	ShowInfo("int_guild: Guilda quebrada (%d)\n",guild_id);
	return 0;
}
...
int mapif_parse_CreateGuild(int fd,int account_id,char *name,struct guild_member *master)
{
	...
	int i=0;
#ifdef NOISY
	ShowInfo("Criando guilda (%s)\n", name);
#endif
	if(search_guildname(name) != 0){
		ShowInfo("int_guild: guilda com o mesmo nome existente [%s]\n",name);
	...
	strcpy(g->position[0].name,"Mestre da Guilda");
	...
	for(i=1;i<MAX_GUILDPOSITION-1;i++) {
		sprintf(g->position[i].name,"Posi鈬o %d",i+1);
	...
	if (!inter_guild_tosql(g,GS_BASIC|GS_POSITION|GS_SKILL)) {
		...
		ShowError("Falha ao criar Guilda %s (Mestre da Guilda: %s)\n", g->name, g->master);
		mapif_guild_created(fd,account_id,NULL);
		aFree(g);
		return 0;
	}
	ShowInfo("Guilda criada %d - %s (Mestre da Guilda: %s)\n", g->guild_id, g->name, g->master);
	...
	if(log_inter)
		inter_log("guilda %s (id=%d) criada pelo mestre %s (id=%d)\n",
...
int mapif_parse_BreakGuild(int fd,int guild_id)
{
	...
	if(log_inter)
		inter_log("guilda %s (id=%d) quebrada\n",g->name,guild_id);
...
int mapif_parse_GuildMemberInfoChange(int fd,int guild_id,int account_id,int char_id,int type,const char *data,int len)
{
	...
	// Not Found
	if(i==g->max_member){
		ShowWarning("int_guild: GuildMemberChange: %d,%d nao encontrado na guilda (%d - %s)\n",
...
int inter_guild_charname_changed(int guild_id,int account_id, int char_id, char *name)
{
	...
	if( g == NULL )
	{
		ShowError("inter_guild_charrenamed: Nao foi possivel encontrar a guilda %d.\n", guild_id);
		return 0;
	}
	...
	if( i == g->max_member )
	{
		ShowError("inter_guild_charrenamed: Nao foi possivel encontrar o personagem %d na guilda\n", char_id);
		return 0;
	}
...
int mapif_parse_GuildCastleDataLoad(int fd,int castle_id,int index)
{
	...
	default:
		ShowError("mapif_parse_GuildCastleDataLoad ERRO!! (Nao encontrado index=%d)\n", index);
		return 0;
	}
}
...
int mapif_parse_GuildCastleDataSave(int fd,int castle_id,int index,int value)
{
	...
	case 1:
		if( gc.guild_id!=value ){
			...
			if(log_inter)
				inter_log("guilda %s (id=%d) %s castelo id=%d\n",
	...
	default:
		ShowError("mapif_parse_GuildCastleDataSave ERRO!! (Nao encontrado index=%d)\n", index);
		return 0;
	}
...
int mapif_parse_GuildMasterChange(int fd, int guild_id, const char* name, int len)
{
	...
	ShowInfo("int_guild: Mestre da Guilda trocado por %s (Guilda %d - %s)\n",g->master, guild_id, g->name);
