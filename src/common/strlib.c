...
// on/off, english, français, deutsch, español, português
//--------------------------------------------------
int config_switch(const char* str)
{
	if (strcmpi(str, "on") == 0 || strcmpi(str, "yes") == 0 || strcmpi(str, "oui") == 0 || strcmpi(str, "ja") == 0 || strcmpi(str, "si") == 0 || strcmpi(str, "sim") == 0)
		return 1;
	if (strcmpi(str, "off") == 0 || strcmpi(str, "no") == 0 || strcmpi(str, "non") == 0 || strcmpi(str, "nein") == 0 || strcmpi(str, "não") == 0 || strcmpi(str, "nao") == 0)
		return 0;
...
int sv_parse_next(struct s_svstate* sv)
{
	...
	{
		ShowError("sv_parse_next: delimitador '\\n' nao e compativel com opcoes SV_TERMINATE_LF ou SV_TERMINATE_CRLF.\n");
		return -1;// error
	}
	if( delim == '\r' && (opt&(SV_TERMINATE_CRLF|SV_TERMINATE_CR)) )
	{
		ShowError("sv_parse_next: delimitador '\\r' nao e compativel com opcoes SV_TERMINATE_CR ou SV_TERMINATE_CRLF.\n");
		return -1;// error
	}
		...
		case PARSING_C_ESCAPE:// skip escape sequence (validates it too)
			{
				...
				if( IS_END() )
				{
					ShowError("sv_parse_next: sequencia de escape em branco\n");
					return -1;
				}
				if( str[i] == 'x' )
				{// hex escape
					...
					{
						ShowError("sv_parse_next: \\x sem digitos hexadecimais em seguida\n");
						return -1;
					}
					do{
						++i;// hex digit
				...
				{
					ShowError("sv_parse_next: sequencia de escape desconhecida \\%c\n", str[i]);
					return -1;
				}
				state = PARSING_FIELD;
				break;
...
int sv_split(char* str, int len, int startoff, char delim, char** out_fields, int nfields, enum e_svopt opt)
{
	...
	{
		ShowError("sv_split: delimitador de linha desconhecido 0x02%x.\n", (unsigned char)end[0]);
		return -1;// error
	}
	++out_fields;
	--nfields;
...
size_t sv_unescape_c(char* out_dest, const char* src, size_t len)
{
	...
	for( i = 0, j = 0; i < len; )
	{
		if( src[i] == '\\' )
		{
			++i;// '\\'
			if( i >= len )
				ShowWarning("sv_unescape_c: sequencia de escape em branco\n");
			else if( src[i] == 'x' )
			{// hex escape sequence
				unsigned char c = 0;
				...
				{
					ShowWarning("sv_unescape_c: \\x sem digitos hexadecimais em seguida\n");
					continue;
				}
				do{
					if( c > 0x0F && inrange )
					{
						ShowWarning("sv_unescape_c: sequencia de escape hexadecimal fora do alcance\n");
						inrange = 0;
					}
...
bool sv_readdb(const char* directory, const char* filename, char delim, int mincols, int maxcols, int maxrows, bool (*parseproc)(char* fields[], int columns, int current))
{
	...
	if( fp == NULL )
	{
		ShowError("sv_readdb: nao pode ler %s\n", path);
		return false;
	}
	...
	while( fgets(line, sizeof(line), fp) )
	{
		...
		if( columns < mincols )
		{
			ShowError("sv_readdb: Colunas insuficientes na linha %d de \"%s\" (%d encontradas, minimo %d necessarias).\n", lines, path, columns, mincols);
			continue; // not enough columns
		}
		if( columns > maxcols )
		{
			ShowError("sv_readdb: Muitas colunas na linha %d de \"%s\" (%d encontradas, maximo e %d).\n", lines, path, columns, maxcols );
			continue; // too many columns
		}
		if( entries == maxrows )
		{
			ShowError("sv_readdb: Alcancado limite de entradas maximo permitido (%d) ao analisar arquivo \"%s\".\n", maxrows, path);
			break;
		}
