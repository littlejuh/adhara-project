...
int Sql_Connect(Sql* self, const char* user, const char* passwd, const char* host, uint16 port, const char* db)
{
	...
	{
		ShowSQL("Falha ao estabelecer keepalive para conexao do BD!\n");
		return SQL_ERROR;
	}
...
int Sql_GetColumnNames(Sql* self, const char* table, char* out_buf, size_t buf_len, char sep)
{
	...
	while( SQL_SUCCESS == Sql_NextRow(self) && SQL_SUCCESS == Sql_GetData(self, 0, &data, &len) )
	{
		...
		{
			ShowDebug("Sql_GetColumns: buffer de saida muito pequeno\n");
			*out_buf = '\0';
			return SQL_ERROR;
		}
...
{
	Sql* self = (Sql*)data;
	ShowInfo("Pingando servidor SQL para manter conexao ativa...\n");
	Sql_Ping(self);
	return 0;
}
...
static int Sql_P_BindSqlDataType(MYSQL_BIND* bind, enum SqlDataType buffer_type, void* buffer, size_t buffer_len, unsigned long* out_length, int8* out_is_null)
{
	...
	case SQLDT_BLOB: bind->buffer_type = MYSQL_TYPE_BLOB;
		break;
	default:
		ShowDebug("Sql_P_BindSqlDataType: tipo de buffer nao suportado (%d)\n", buffer_type);
		return SQL_ERROR;
	}
	bind->buffer = buffer;
...
static void SqlStmt_P_ShowDebugTruncatedColumn(SqlStmt* self, size_t i)
{
	...
	ShowSQL("Erro de BD - dados do campo '%s' foram truncados.\n", field->name);
	ShowDebug("column - %lu\n", (unsigned long)i);
	...
	if( column->buffer_type == MYSQL_TYPE_STRING )
		Sql_P_ShowDebugMysqlFieldInfo("buffer - ", column->buffer_type, column->is_unsigned, column->buffer_length, "+1(terminador-nul)");
	else
...
int SqlStmt_BindColumn(SqlStmt* self, size_t idx, enum SqlDataType buffer_type, void* buffer, size_t buffer_len, uint32* out_length, int8* out_is_null)
{
	...
	if( buffer_type == SQLDT_STRING || buffer_type == SQLDT_ENUM )
	{
		if( buffer_len < 1 )
		{
			ShowDebug("SqlStmt_BindColumn: buffer_len(%d) muito pequeno, nao ha espaco para o terminador-nul\n", buffer_len);
			return SQL_ERROR;
		}
...
int SqlStmt_NextRow(SqlStmt* self)
{
	...
	if( err == MYSQL_DATA_TRUNCATED )
	{
		...
		if( !self->bind_columns )
		{
			ShowSQL("Erro de BD - dados truncados (fonte desconhecida, colunas nao estao delimitadas)\n");
			return SQL_ERROR;
		}
		...
		ShowSQL("Erro de BD - dados truncados (fonte desconhecida)\n");
		return SQL_ERROR;
	}
#endif
	if( err )
